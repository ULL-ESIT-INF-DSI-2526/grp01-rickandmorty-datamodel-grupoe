...
describe('GestorEspecies - Pruebas Unitarias', () => {
  ...
  it ('debería agregar una especie correctamente', async () => {
    ...
    const nuevaEspecie: Species = { id: 'sp-1', name: 'Humano', origin: 'Tierra', type: 'amorfo', expectancy: 80, description: '...' };
  ...
  it ('debería lanzar un error al intentar agregar una especie con un ID ya existente', async () => {
    const especie1: Species = { id: 'sp-1', name: 'Humano', origin: 'Tierra', type: 'humanoide', expectancy: 80, description: '...' };
    const especie2: Species = { id: 'sp-1', name: 'Alienígena', origin: 'Planeta', type: 'amorfo', expectancy: 150, description: '...' };
    
    await gestor.agregarEspecie(especie1);
    await expect(gestor.agregarEspecie(especie2)).rejects.toThrow(`Ya existe una especie con el ID ${especie2.id}`);
  });

  it ('debería eliminar una especie correctamente', async () => {
    const especie: Species = { id: 'sp-1', name: 'Humano', origin: 'Tierra', type: 'humanoide', expectancy: 80, description: '...' };
  ...
  it ('debería lanzar un error al intentar eliminar una especie que no existe', async () => {
    await expect(gestor.eliminarEspecie('sp-999')).rejects.toThrow(`No existe una especie con el ID sp-999`);
  });

  it('debería devolver una lista con todas las especies registradas', async () => {
    const especie1: Species = { id: 'sp-1', name: 'Humano', origin: 'Tierra', type: 'humanoide', expectancy: 80, description: '...' };
    const especie2: Species = { id: 'sp-2', name: 'Alienígena', origin: 'Planeta X', type: 'amorfo', expectancy: 150, description: '...' };
    const especie3: Species = { id: 'sp-3', name: 'Robot', origin: 'Artificial', type: 'mecánico', expectancy: 1000, description: '...' };
